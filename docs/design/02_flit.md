...
AW/AR payload 不重複儲存 qos，header 為唯一來源。Response flit 繼承對應 request 的 `qos`。Router arbiter 以 `qos` 為第一優先級比較依據。詳見 [QoS Design](07_qos.md)。
...
當 `commtype=1`（Multicast）或 `commtype=2`（ParallelReduction）時，`multicast_mask`（16 bits）採用 **RCR (Rectangle Coordinate-Range)** 編碼，以 `(min, max)` 座標對表達任意軸對齊矩形，不受 power-of-2 對齊限制。詳見 [RCR-Multicast Spec](10_multicast.md)。
...
- [QoS Design](07_qos.md)
- [Router Specification](03_router.md)
- [Network Interface Specification](04_network_interface.md)
- [Physical Channel Architecture](06_physical_channel.md)
- [Fixed-Width Channel Mode](A1_fixed_width_analysis.md)
- [Width Converter](09_width_converter.md)
