...
| `HEADER_WIDTH` | 48 | Header width (both versions) |
| `PAYLOAD_WIDTH` | 352 | Maximum payload width (W/R channels) |
| `FLIT_WIDTH` | 400 | Total flit width (Header + Payload) |
...
```
  47                                        0   351                              0
  ┌────────────────────────────────────────────┬──────────────────────────────────┐
  │               Header (48 bits)             │        Payload (352 bits)        │
  └────────────────────────────────────────────┴──────────────────────────────────┘
  |<──────────────────────── Flit: 400 bits ────────────────────────────────>|
```

所有 AXI channel 共用 400-bit flit width，較短 payload 以 zero-padding 對齊至 352 bits。Request 與 Response flit 寬度對稱。
...
## 2. Header Format (48 bits)

Header 固定 48 bits，提供 Valid/Ready mode（No-VC）與 Credit-Based mode（With-VC）兩種配置。兩者僅 bit [34:32] 的 vc_id/rsvd 配置不同，其餘欄位位置與寬度完全相同。Reserved bits 統一置於 MSB 端。
...
| [22:15] | dst_id | 8 | Destination node ID |
...
| [34:32] | vc_id ‡ | 3 | Virtual Channel ID (0\~7) |
| [47:35] | rsvd ‡ | 13 | Reserved |
| | **Total** | **48** | |

**‡ Version-dependent fields:**

- **Valid/Ready mode (No-VC):** bit [47:32] 全為 rsvd（16b），無 vc_id。VC 由 signal-level 多組 valid/ready lines 實現。
- **Credit-Based mode (With-VC):** bit [34:32] 為 vc_id（3b），bit [47:35] 為 rsvd（13b）。VC ID 編碼於 header，搭配 credit-based flow control。
...
#### 2.2.7 VC ID & Reserved

**vc_id (3 bits, Credit-Based mode only):**

支援最多 8 個 Virtual Channels（bit [34:32]）。用途含 Request/Response 分離避免 deadlock、traffic class 隔離。Valid/Ready mode 中此 3 bits 歸入 reserved。

**Reserved bits:**

| Mode | Bit Range | Width |
|------|-----------|-------|
| Valid/Ready (No-VC) | [47:32] | 16 |
| Credit-Based (With-VC) | [47:35] | 13 |

Reserved 統一集中於 MSB 端，傳輸時設為 0，接收端忽略。
...
| 6 | VC / Reserved | vc_id, rsvd | Version-dependent |

高頻存取欄位置於 LSB 端，第一級 pipeline 僅需 qos + axi_ch 即可完成 arbitration 與 channel 分離。Reserved 置於 MSB 端，未來擴展不影響既有 bit position。
...
```
 47              32 31    27  26   25  24 23 22    15 14     7  6  4  3  0
┌─────────────────┬──────┬────┬────┬─────┬────────┬────────┬─────┬──────┐
│      rsvd       │ rob  │rob │last│port │ dst_id │ src_id │ axi │ qos  │
│      16b        │idx 5b│req │ 1b │id 2b│   8b   │   8b   │ch 3b│  4b  │
│                 │      │ 1b │    │     │        │        │     │      │
└─────────────────┴──────┴────┴────┴─────┴────────┴────────┴─────┴──────┘
```
...
```
 47    35 34 32 31    27  26   25  24 23 22    15 14     7  6  4  3  0
┌──────┬─────┬──────┬────┬────┬─────┬────────┬────────┬─────┬──────┐
│ rsvd │vc_id│ rob  │rob │last│port │ dst_id │ src_id │ axi │ qos  │
│ 13b  │  3b │idx 5b│req │ 1b │id 2b│   8b   │   8b   │ch 3b│  4b  │
│      │     │      │ 1b │    │     │        │        │     │      │
└──────┴─────┴──────┴────┴────┴─────┴────────┴────────┴─────┴──────┘
```
...
| b_rsvd | [63:19] | 45 | Reserved |
| | | **64** | |

`ecc_fail` 為 NoC 內部欄位，NI 記錄於 CSR，AXI interface 透過 `bresp` 回報（錯誤 → SLVERR）。`buser` 為 AXI user signal pass-through。
...
| flit | [401:2] | 400 | Flit data (req or rsp) |
| | | **402** | |
...
| Wires per link | NumVC × 402 | 402 (shared) |
| Wire count (4 VC) | 1,608 wires/dir | 402 wires/dir |
...
- **Dual Physical Network** — Req/Rsp link (402b each)
  - 獨立 full-duplex link，消除 Req-Rsp circular dependency（protocol deadlock avoidance）
  - 每方向每 cycle 32 bytes，Req/Rsp 對稱且 flit 寬度統一（400b）
...
- **Header integrity** — ECC 僅保護 payload data（wdata/rdata），不涵蓋 48-bit header 及 payload metadata（wstrb、axi_id 等）。Header integrity 依賴 physical link layer（wire-level parity 或 CRC）。`dst_id` bit flip 將導致 misrouting
...
- [QoS Design](06_qos.md) — Header qos 欄位使用方式
- [Router Specification](03_router.md) — 400-bit flit 轉發與 pipeline
- [Network Interface Specification](04_network_interface.md) — AXI ↔ flit conversion
- [Physical Channel Architecture](05_physical_channel.md) — 2-channel / 3-channel link 架構
- [Width Converter](10_width_converter.md) — AXI 寬度轉換與 payload 對應
