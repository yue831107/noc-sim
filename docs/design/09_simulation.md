...
本文件定義 NoC C++ model 的模擬平台架構、配置參數、I/O pattern、public API、cycle model 與可替換邊界。
...
| **RTL co-simulation golden reference** | C++ output 作為 RTL 驗證的比對基準 | Memory State + Response Log |
...
│   6 Groups: Construction / Transaction / SimControl /               │
│             Completion / Metrics / Debug                            │
└────────────────────────────┬────────────────────────────────────────┘
                             ↓ delegates to
┌─────────────────────────────────────────────────────────────────────┐
│ Internal Components                                                  │
...
│   │  CppRouter ↔ RouterDpiBridge  |  CppNI ↔ NIDpiBridge       │   │
│   └─────────────────────────────────────────────────────────────┘   │
└────────────────────────────┬────────────────────────────────────────┘
                             ↓ DPI-C
┌─────────────────────────────────────────────────────────────────────┐
...
| `MESH_COLS` | 4 | Mesh 欄數 |
| `MESH_ROWS` | 4 | Mesh 列數 |
| `DEFAULT_LOCAL_PORTS` | 1 | 預設每 Router 的 LOCAL port 數 (0~4) |
| `NODE_LOCAL_PORTS` | `{}` | Per-node override（node_id → local port 數量） |
| **Buffer** | | |
| `INPUT_BUFFER_DEPTH` | 4 | Router input buffer 深度 (flits) |
...
| `SW_ALLOC_DELAY` | 0 | SA stage 額外 cycles |
| `CREDIT_DELAY` | 1 | Credit return latency (cycles) |
| `CHANNEL_DELAY` | 1 | Link propagation latency (cycles) |
| **Allocator** | | |
| `VC_ALLOCATOR` | `"round_robin"` | VC allocator 策略（`round_robin` / `islip`） |
| `SW_ALLOCATOR` | `"qos_aware_rr"` | Switch allocator 策略（`qos_aware_rr` / `islip`） |
| **Traffic** | | |
...
預載 HostMemory 和各 node 的 LocalMemory 初始內容。使用 `$readmemh` 格式（`.hex`）。
...
| `inject_cycle` | 絕對 cycle 數 或 `"after:<txn_id>"` |

### 4.4 Output: Memory State

所有 transaction 完成後各 node LocalMemory dump。`.hex` 格式，byte-level exact match。
...
| F | Debug & Output | get_router / dump_state / generate_golden |

### 5.2 Group A: Construction & Configuration

| Function | Description |
|----------|-------------|
| `NocSystem(const NocConfig&)` | 建構系統 |
| `load_config(json_path) → NocConfig` | 從 JSON 載入配置（static） |
| `load_memory(hex_dir)` | 載入 Memory Init |
| `load_traffic(json_path)` | 載入 Traffic |
| `load_host_memory(addr, data, len)` | 直接寫入 HostMemory |
| `load_local_memory(node_id, addr, data, len)` | 直接寫入 LocalMemory |

### 5.3 Group B: Transaction Submission

回傳 `TxnHandle`（uint32_t），用於追蹤 completion。
...
### 5.5 Group D: Completion & Response
...
| Function | Description |
|----------|-------------|
| `get_metrics()` | 取得 MetricsCollector |
...
C++ model 將 RTL 的並行行為拆為 8 個循序 phase，以 ordering 保證因果正確：

| RTL posedge clk（Sequential） | C++ Phase |
|-------------------------------|-----------|
| Input FF latch | Phase 1 |
| Output FF update | Phase 6 |
| Credit counter update | Phase 7 |

| RTL combinational | C++ Phase |
|-------------------|-----------|
...
## 8. TrafficManager（中央協調器）

TrafficManager 位於 NocSystem API 與 Mesh 之間，管理 transaction 的完整生命週期。

### 8.1 架構定位

```
NocSystem Public API
...
TrafficManager 的 `tick(current_cycle)` 每 cycle 執行：
...
| 事件 | TrafficManager 動作 | Scoreboard 動作 |
|------|--------------------|--------------------|
...
Router 和 NI 均實作抽象介面，Simulation Driver 透過 wiring loop 連接所有元件，無需區分 C++ 或 RTL 實作。

```
┌──────────────┐                           ┌──────────────┐
│  CppRouter   │── Router_Interface<M> ──│RouterDpiBridge│
│  (C++ Model) │                           │  (DPI-C)     │
└──────────────┘                           └──────────────┘
         ▲           Simulation Driver           ▲
         │           (Mesh wiring loop)          │
         ▼                                       ▼
┌──────────────┐                           ┌──────────────┐
│  CppNI       │── NI_Interface<M> ──────│  NIDpiBridge  │
│  (C++ Model) │                           │  (DPI-C)     │
└──────────────┘                           └──────────────┘
```
...
|-------------------|--------|----|----------|
| Pure C++ | CppRouter | CppNI | 高速效能模擬 |
| NI RTL | CppRouter | NIDpiBridge | NI RTL 驗證 |
| Router RTL | RouterDpiBridge | CppNI | Router RTL 驗證 |
| Full RTL | RouterDpiBridge | NIDpiBridge | 系統級 co-sim |

**Hot-Swap 約束：**
- 替換前必須 drain 目標元件的所有 Channel pipeline（排空 in-flight flits）
- 不支援 in-flight flit 的跨實作遷移（C++ state ↔ RTL register 格式不同）
...
| Scoreboard / MetricsCollector | 驗證（online per-txn tracking）與統計邏輯，C++ only |
...
DPI-C bridge 為 thin wrapper，提供 SystemVerilog 與 C++ model 的互操作介面。

### 10.1 Transaction-Level API

用於 C++ model 獨立執行，產生 golden output。SV testbench 只做 submit + step + response。

| Function | Description |
|----------|-------------|
| `noc_init(json_path) → handle` | 建立 NocSystem |
...
用於 hot-swap co-sim。RTL proxy 每 cycle 透過 signal-level API 讀寫 port 信號，與 C++ mesh 中的其他元件互動。

| Function | Description |
|----------|-------------|
| `noc_set_port_input(h, node, port, ch, valid, flit, ready_or_credit)` | RTL → C++ model |
...
| 模式 | Memory 歸屬 | 同步機制 |
|------|-----------|---------|
| C++ as golden | C++ model 維護 | RTL memory 由 C++ shadow |
| RTL as primary | RTL memory 為 primary | C++ 每 cycle 讀取 RTL state |
...
- [Verification](10_verification.md) — Golden 驗證、效能指標、測試策略
- [Architecture Diagram](../images/noc_env.txt) — pcievhost 風格系統架構圖
