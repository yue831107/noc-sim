...
C++ model 將 RTL 的並行行為拆為 8 個循序 phase，以 ordering 保證因果正確（詳細 Phase 4 state machine 與 credit 時序見 [Simulation Platform](08_simulation.md) §6）：
...
| **D: Metrics** | `get_metrics()` | 取得 MetricsCollector 參照 |
| | `verify()` | 執行 golden 驗證 |
...
| B: Transaction | ← main()、→ Callback (`complete/data`) | → TrafficManager (`submit`)、← TrafficManager (`is_complete/get_read_data`) | submit_write / submit_read / completion 查詢 |
| C: Simulation | ← main() | → TrafficManager (`tick()`) | run(N) / run_all → 每 cycle 驅動 |
| D: Metrics | — | → Scoreboard (`verify()`)、→ MetricsCollector (`get_metrics()`)（虛線） | 驗證與效能查詢 |
...
| 11 | TrafficManager | Group B | complete/data | 反向：完成結果回傳 API |
| 12 | Group D | Scoreboard | verify() | 虛線，觸發 golden 比對 |
| 13 | Group D | MetricsCollector | get_metrics() | 虛線，查詢效能統計 |
