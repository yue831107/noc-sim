...
完整架構圖見 [noc_architecture.svg](../images/noc_architecture.svg)。以下定義每個元件的 public / internal function。
...
完整系統架構圖（包含所有元件 function 定義、data flow、Hot-Swap boundary）：

![NoC Architecture](../images/noc_architecture.svg)

架構圖由左到右分為 4 欄，對應 User Code → Public API → Internal Components → Co-Sim Bridge。以下逐欄說明。
