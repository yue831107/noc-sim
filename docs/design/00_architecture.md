...
```
  ...
  │  │                                                       │  │
  │  │  AXI Slave ──► AddrTrans ──► QoSGenerator ──► FlitPacker     │  │──► Req Router
  │  │  (AW/AR/W)     (XY offset    (Bypass/   (AXI→Flit    │  │
  │  │                 or SAM)      Fixed/      + ECC Gen)   │  │
  │  │                              Limiter/                 │  │
  │  │  AXI Slave ◄── RoB ◄──────── FlitUnpacker ◄── ECC Chk  │  │◄── Rsp Router
  ...
  │  │                                                       │  │
  │  │  Req Router ──► FlitUnpacker ──► ReqInfoStore           │  │
  │  │                 (Flit→AXI)     (Save src/qos/rob_idx) │  │
  │  │                                      │                │  │
  │  │                                      ▼                │  │
  │  │  Rsp Router ◄── FlitPacker ◄── AXI Response ◄── Memory │  │
...
```
  ...
  │ AddrTrans   │    │                                    │    │ FlitUnpacker  │
  │ QoSGenerator      │───►│ [AW flit]──►Router──►Router──►───│───►│ Reassembly  │
  │ FlitPacker    │    │ [W flit 0]──►  ↓       ↓    ───│───►│ → AXI W     │
  ...
  │ FlitUnpacker  │◄──│◄───Router◄───Router◄──[B flit]◄──│◄───│ FlitPacker    │
...
```
  ...
  ┌─────────────┐    ┌──────────────────────────────────┐    ┌─────────────┐
  │     NMU     │    │          Request Network          │    │     NSU     │
  │ FlitPacker    │───►│ [AR flit]──►Router──►Router──►───│───►│ FlitUnpacker  │
  ...
  ┌─────┴───────┐    ┌──────────────────────────────────┐    ┌─────────────┐
  │     NMU     │    │         Response Network          │    │     NSU     │
  │ FlitUnpacker  │◄──│◄───Router◄───Router◄──[R flit 0] │◄───│ FlitPacker    │
...
| **pipeline** | `_input_queuing()` | Sample inputs → push to InputBuffer |
...
| | `_memory_op(req)` | 執行 LocalMemory read/write |
...
| | Sub-Modules | InputBuffer、BufferState、RouteComputer、Crossbar、PathLock、Allocator、MulticastTracker、ReductionSync/Arb |
