...
本文件以圖表為主，提供 NoC Cycle-Approximate Model 的全局架構視圖。

本專案建立 **NoC Cycle-Approximate Model**（CA Model），服務兩大目標：

| 目標 | 說明 |
|------|------|
| **Pre-silicon Performance Evaluation** | 高速模擬 NoC traffic，評估 latency / throughput / buffer occupancy |
| **RTL Co-simulation Golden Reference** | CA Model 的 output 作為 RTL 驗證的 bit-exact / cycle-accurate 比對基準 |
...
> **Terminology**：本文件中「CA Router」和「CA NI」指 Cycle-Approximate Model 實作的元件（相對於 RTL 實作）。「CA Model」泛指整個 Cycle-Approximate Model。語言層面的 C++（如 C++ API、C++ test program）維持原稱。命名規範詳見 [Naming Convention](naming_convention.md)。
...
每個 Router 由兩個結構對稱的 Req Router 與 Rsp Router 組成，分別處理 Request 與 Response traffic：
...
### 3.1 CA Model Software / Hardware Architecture
...
CA Model 與 RTL 的替換邊界定義在 `Router_Interface<Mode>` 和 `NI_Interface<Mode>`：

```
  Standalone Simulation                     RTL Co-Simulation (Hot-Swap)

  ┌─────────┐  ┌─────────┐              ┌─────────┐  ┌─────────────┐
  │CA Router│  │CA Router│              │CA Router│  │Router DPI Bridge│
  └────┬────┘  └────┬────┘              └────┬────┘  └──────┬───────┘
       │ Channel    │                        │ Channel      │ DPI-C
  ┌────┴────┐  ┌────┴────┐              ┌────┴────┐  ┌──────┴───────┐
  │  CA NI  │  │  CA NI  │              │  CA NI  │  │  RTL NI (SV) │
  └─────────┘  └─────────┘              └─────────┘  └──────────────┘

  全部 CA 實作                                混合：一個 Router 替換為 RTL
  高速模擬                                驗證 RTL Router 正確性
```

Interface 合約確保 CA 與 RTL 實作的行為一致。替換粒度為單一 Router 或 NI，不需改動 Mesh wiring。
...
同一組 input pattern 分別餵入 CA Model 和 RTL，CA Model output 為 golden，與 RTL output 比對：
...
```
  Mode 1: Standalone Simulation               Mode 2: NI RTL Co-Simulation
  ┌──────────────────────────────┐         ┌──────────────────────────────┐
  │  All CA Router + All CA NI  │         │  All CA Router              │
  │  ───────────────────────     │         │  + NI DPI Bridge (DPI-C ↔ SV) │
  │  No RTL, maximum speed      │         │  Verify NI RTL              │
  └──────────────────────────────┘         └──────────────────────────────┘

  Mode 3: Router RTL Co-Simulation         Mode 4: Full RTL Co-Simulation
  ┌──────────────────────────────┐         ┌──────────────────────────────┐
  │  Router DPI Bridge (DPI-C ↔ SV)│         │  All Router DPI Bridge         │
  │  + All CA NI                │         │  + All NI DPI Bridge           │
  │  Verify Router RTL          │         │  Full system co-simulation  │
  └──────────────────────────────┘         └──────────────────────────────┘
```

### 5.2 DPI-C Bridge 架構（Signal-Level）

CA Model 為主控方。當某個 CA Router / CA NI 被替換為 RTL 時，DPI-C Bridge 透過 Signal-Level DPI-C 每 cycle 與 RTL module 交換 port 信號：

```
  NoC CA Model (主控)                      SystemVerilog Simulator
  ┌──────────────────────┐                  ┌──────────────────────┐
  │                      │   Signal-Level   │                      │
...
### 7.5 CA Router（Router_Interface\<Mode\>）
...
| | `_sw_alloc_evaluate()` | Switch arbitration（pluggable: FIFO/Fixed/RR/Rotate） |
...
| Pluggable Arbitration | 4 種仲裁策略（FIFO/Fixed/RR/Rotate） | [QoS](06_qos.md) |
...
**層 2：CA Router + CA NI 展開**

並排兩個大方塊，展示 CA 實作的階層結構：
...
- **HOT-SWAP BOUNDARY**（橘色虛線框）：CA Router/CA NI 可替換為 Router DPI Bridge/NI DPI Bridge，使用相同的 `Interface<Mode>`
...
| **用途** | 替換 CA Router/CA NI 為 RTL 時，proxy **每 cycle** 與 RTL module 交換 port 信號 |
...
| 19 | CA Router | CA NI | Channel\<T\> | 橘色，Node 內部連線 |
