...
```
  ...
  ┌─────────┐  ┌─────────┐              ┌─────────┐  ┌─────────────┐
  │C++ Router│  │C++ Router│              │C++ Router│  │Router DPI Bridge│
  └────┬────┘  └────┬────┘              └────┬────┘  └──────┬───────┘
       │ Channel    │                        │ Channel      │ DPI-C
  ┌────┴────┐  ┌────┴────┐              ┌────┴────┐  ┌──────┴───────┐
  │  C++ NI  │  │  C++ NI  │              │  C++ NI  │  │  RTL NI (SV) │
  └─────────┘  └─────────┘              └─────────┘  └──────────────┘
...
```
  ...
  ┌──────────────────────────────┐         ┌──────────────────────────────┐
  │  All C++ Router + All C++ NI  │         │  All C++ Router              │
...
C++ model 為主控方。當某個 C++ Router/C++ NI 被替換為 RTL 時，DPI-C Bridge 透過 Signal-Level DPI-C 每 cycle 與 RTL module 交換 port 信號：
...
### 7.5 C++ Router（Router_Interface\<Mode\>）
...
**層 2：C++ Router + C++ NI 展開**

並排兩個大方塊，展示 C++ 實作的階層結構：

| 元件 | 層次 | 內容 |
|------|------|------|
| **C++ Router** | Public | `tick()` Phase 1-4、`post_wire()` Phase 6-7、`get_output()/set_input()` |
...
| **C++ NI** | Public | `tick()` Phase 8、`get_output()/set_input()` |
...
- **HOT-SWAP BOUNDARY**（橘色虛線框）：C++ Router/C++ NI 可替換為 Router DPI Bridge/NI DPI Bridge，使用相同的 `Interface<Mode>`
...
| **用途** | 替換 C++ Router/C++ NI 為 RTL 時，proxy **每 cycle** 與 RTL module 交換 port 信號 |
...
| 19 | C++ Router | C++ NI | Channel\<T\> | 橘色，Node 內部連線 |
