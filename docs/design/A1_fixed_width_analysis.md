...
本文件說明 NoC 採用統一 408-bit 固定寬度 flit 的設計動機、結構細節、效率分析，以及與 per-channel variable-width 方案的比較。所有參數與欄位定義以 [Flit Format](02_flit.md) 為準。
...
> **⚠ 注意：** 本 section 的 header bit allocation 為早期設計版本，與 [04_flit.md](02_flit.md) 的最終定義不一致。所有 header 欄位定義以 04_flit.md Section 2 為準。本文的固定寬度效率分析結論仍然有效。

Header 固定 56 bits，提供兩個版本（No-VC / With-VC），詳見 [04_flit.md Section 2](02_flit.md)。以下為 bit allocation 摘要：
...
> **⚠ 注意：** 以下 bit layout 為早期設計版本，欄位名稱與排列已過時。最終 header bit layout 請參見 [04_flit.md Section 2.1](02_flit.md)。
...
- [Physical Channel Architecture](06_physical_channel.md) — 雙通道架構詳述
- [Width Converter](09_width_converter.md) — 不同 AXI bus 寬度轉換
