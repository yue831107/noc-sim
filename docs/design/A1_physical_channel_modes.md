...
所有參數與 [Flit Format](04_flit.md) 一致：
...
非 reserved 欄位佔 352-bit payload 的比例（資料來自 [Flit Format](04_flit.md) Section 3.7）：
...
Narrow flit 使用與 [Flit Format](04_flit.md) **相同的 56-bit header**，payload 縮小為 108 bits：

```
  163                                              108 107                      0
  ┌──────────────────────────────────────────────────┬──────────────────────────┐
  │              Header (56 bits)                     │    Payload (108 bits)    │
  │  與 04_flit.md Section 2 完全一致                  │    (channel-dependent)   │
  └──────────────────────────────────────────────────┴──────────────────────────┘
```
...
與現有 [Flit Format](04_flit.md) **完全相同**（56-bit header + 352-bit payload）。Wide channel 承載 WideAW、WideW、WideR，payload 格式不變。
...
- [Flit Format](04_flit.md) — Flit 結構、header 欄位定義、payload format、physical link format
- [Router Specification](02_router.md) — Router pipeline、ReqRouter/RspRouter 架構、wormhole switching
- [Network Interface Specification](03_network_interface.md) — NI 的 AXI-to-Flit 轉換與 network 分流
- [Internal Interface](05_internal_interface.md) — Abstract Interface、8-phase cycle 定義
- [QoS Design](10_qos.md) — QoS-Aware arbitration policy
