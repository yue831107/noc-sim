...
Router 是 NoC Mesh 的封包轉發單元，負責接收 flit、路由決策、仲裁、crossbar switching 與轉發。每個 Router 由兩個結構對稱的 Req Router / Rsp Router 組成 — **Req Router**（AW/W/AR）與 **Rsp Router**（B/R），共用相同的 routing、arbitration、crossbar 邏輯。
...
- Cycle-approximate CA Model，可與 RTL co-simulation
- XY Routing 保證 deadlock-free
- Wormhole switching 保證封包完整性
- Pluggable arbitration 支援 4 種仲裁策略（FIFO/Fixed/RR/Rotate）
...
```
 ...
 Each port data path:
   Ingress → Input Buffer(depth=INPUT_BUFFER_DEPTH) → Crossbar → Output Buffer(depth=OUTPUT_BUFFER_DEPTH*) → Egress
...
| Sub-Module | 職責 |
|------------|------|
| Input Buffer | Per-port, per-VC flit 儲存。Push/Pop/Peek |
| Buffer State | Per-{output_port, vc} downstream credit counter（sender side），追蹤對端 input buffer 可用空間 |
| Route Computer | XY routing |
| VC Allocator | HEAD flit 的 output VC 分配（pluggable） |
| SW Allocator | Per-output switch arbitration（pluggable: FIFO/Fixed/RR/Rotate） |
| Crossbar | N×N non-blocking switch fabric |
| Path Lock | Per-{input_port, vc} wormhole lock FSM |

### 2.3 Req Router / Rsp Router

每個邏輯 Router 包含兩個獨立的 Req Router / Rsp Router，使用獨立 physical link，消除 request-response circular dependency：
...
以下參數對應 RTL module 的 `parameter`，在 CA Model 中為 compile-time 常數或 NocConfig 配置值：
...
| Value | RTL 對應 | Description | 特性 |
|-------|----------|-------------|------|
| `VALID_READY` | `VcNaive` | Valid/Ready handshake | 最簡單，可能產生跨模組 combinational path |
| `CREDIT` | `VcCredit` | Credit-based flow control | 切斷 in-to-out path，需 INPUT_BUFFER_DEPTH ≥ 3 |

> **Note：** RTL `VcPreemptValid` 為 `VALID_READY` 的 timing 變體（`credit_o` 同樣恆為 1），歸類於 `VALID_READY`。VC 數量由 `NUM_VIRT_CHANNELS` 獨立控制，與 `FLOW_CONTROL` 正交。
...
| `id_t` | Node ID type（`logic[NODE_ID_WIDTH-1:0]`） | NODE_ID_WIDTH 推導 |
| `addr_rule_t` | Address routing rule（IdTable 用） | 上層定義 |
| `payload_t` | Flit payload type（352 bits） | [Flit Format](02_flit.md) |

### 3.5 Simulation Parameters（CA Model 專用）

以下參數僅存在於 CA Model，用於 pipeline delay 模擬，無直接 RTL 對應：
...
- `INPUT_BUFFER_DEPTH % NUM_VIRT_CHANNELS == 0`（必須整除）
- `INPUT_BUFFER_DEPTH_PER_VC = INPUT_BUFFER_DEPTH / NUM_VIRT_CHANNELS`
- `FLOW_CONTROL == CREDIT → INPUT_BUFFER_DEPTH ≥ 3`（credit round-trip latency 需要）
- `NUM_ROUTES ≤ 8`（4 mesh direction + max 4 LOCAL port）
...
|--------|-----------|-------|-------------|
| `valid_i` | [NUM_INPUT][NUM_VIRT_CHANNELS] | 1 bit each | Per-VC flit valid |
| `ready_o` | [NUM_INPUT][NUM_VIRT_CHANNELS] | 1 bit each | Per-VC buffer ready |
| `data_i` | [NUM_INPUT][NUM_PHYS_CHANNELS] | FLIT_WIDTH each | Flit data（shared across VCs） |
| `credit_o` | [NUM_INPUT][NUM_VIRT_CHANNELS] | 1 bit each | Per-VC credit return（CREDIT mode 時使用） |

**Output Channels（to downstream）：**

| Signal | Dimension | Width | Description |
|--------|-----------|-------|-------------|
| `valid_o` | [NUM_OUTPUT][NUM_VIRT_CHANNELS] | 1 bit each | Per-VC flit valid |
| `ready_i` | [NUM_OUTPUT][NUM_VIRT_CHANNELS] | 1 bit each | Per-VC downstream ready |
| `data_o` | [NUM_OUTPUT][NUM_PHYS_CHANNELS] | FLIT_WIDTH each | Flit data |
| `credit_i` | [NUM_OUTPUT][NUM_VIRT_CHANNELS] | 1 bit each | Per-VC credit from downstream |

**Control Inputs：**

| Signal | Width | Description |
|--------|-------|-------------|
| `xy_id_i` | NODE_ID_WIDTH | 本 Router 的 XY 座標（XYRouting 使用） |
| `id_route_map_i` | NUM_ADDR_RULES × addr_rule_t | Routing table（IdTable 使用） |

### 4.3 Flow Control 行為

Flow control 行為由 `FLOW_CONTROL` parameter 決定：

**VALID_READY（對應 RTL VcNaive / VcPreemptValid）：**
- Handshake 成立：`valid_i[in][vc] && ready_o[in][vc]`
- `ready_o` 由 input buffer 的可用空間決定（`!buffer.full()`）
- `credit_o` 恆為 1（不使用）

**CREDIT（對應 RTL VcCredit）：**
- Sender 在 `credit_counter > 0` 時發送，不依賴 `ready_i`
- `ready_o` 仍反映 input buffer 可用空間（default 為高），但 upstream sender 不使用此信號做 flow control
- `credit_o` 為 **registered output**：handshake 成立（`valid_i & ready_o`）後的**下一 cycle** assert，持續 1 cycle（1-cycle latency）
- Credit counter 初始值 = `INPUT_BUFFER_DEPTH_PER_VC`
- Invariant: `0 ≤ credit_counter[vc] ≤ INPUT_BUFFER_DEPTH_PER_VC`

**Output Side Credit 處理（兩種模式共用）：**
- Router 收到 `credit_i[out][vc]` 時，更新 Buffer State 的 credit counter：`credit_counter[out][vc]++`
- 此更新發生在 Phase 7（Credit Update），見 FR-07
...
Reset 後所有 register 回到初始狀態：

| Register / State | Reset Value | 說明 |
|-----------------|-------------|------|
| Path Lock `locked` | `UNLOCKED` (0) | 所有 {input_port, vc} 均無鎖定 |
| Credit counter（sender side） | `INPUT_BUFFER_DEPTH_PER_VC` | 假設對端 buffer 全空 |
| Input Buffer | Empty | 所有 slot 清空，`occupancy = 0` |
| Output Buffer | Empty | 所有 slot 清空 |
| `valid_o` | 0 | 無 output flit |
| `ready_o` | 1 | Buffer 空 → ready |
| `credit_o` | 0 | 無 pending credit return |
...
**Invalid destination：** `dst_id` 超出 mesh 範圍時 drop HEAD flit + log warning。若為 multi-flit wormhole packet（HEAD 的 `last=0`），後續 body/tail flit 一併 drain（消費並丟棄），直到收到 `last=1` 的 TAIL flit，避免無 path lock 的 body/tail 被誤判為新 HEAD。

> **Testability：** 以 4×4 mesh 測試所有 5 方向 output（N/S/E/W/LOCAL），驗證 edge node 不路由至不存在方向，注入 invalid `dst_id` 驗證 drop + warning。注入 invalid dst 的 multi-flit burst，驗證 body/tail 全部 drain。
...
**Path Lock（per-{input_port, vc} 1-bit register `locked`）：**
...
每個 `{input_port, vc}` 有獨立的 path lock register，允許同一 input port 的不同 VC 同時鎖定不同 output port。鎖定期間 `route_sel` 保持不變（使用 registered value），BODY/TAIL 不參與 routing 與 arbitration。
...
> **Testability：** 驗證 Path Lock FSM 所有 4 種轉移（UNLOCKED↔LOCKED），測試 single-flit（HEAD+TAIL）不鎖路，測試 multi-flit burst 鎖路與 TAIL 釋放。
...
| 7 | Credit Update | 收到 credit return 後更新 counter（CREDIT mode） |
| 8 | NI Process | NI 處理 AXI ↔ flit conversion |

> Phase 8（NI Process）不屬於 Router，見 [Network Interface](04_network_interface.md)。完整 8-phase 定義見 [Simulation Platform](08_simulation.md) §6（single source of truth）。
...
| 4b VA | VC Allocation | Output VC 分配（pluggable Allocator） |
| 4c SA | Switch Allocation | Switch arbitration（pluggable: FIFO/Fixed/RR/Rotate） |
...
**描述：** Per-output pluggable 仲裁，支援 4 種策略。

**4 種仲裁策略（由 NocConfig 選擇）：**

| Scheme | 行為 |
|--------|------|
| FIFO | 依 request 到達順序，per-output FIFO queue 記錄 requesting input port（預設） |
| Fixed Priority | 依 `qos` 值高者優先（qos=15 highest） |
| Round Robin | Per-output RR pointer 輪詢 |
| Rotate | 類似 RR，每次 grant 後 pointer 旋轉至下一 input |

Wormhole locked BODY/TAIL 不參與仲裁，直接通過。
...
**Anti-starvation：** Round Robin / Rotate 策略天然保證公平性，HEAD flit 不會被同 level 的其他 HEAD 無限期 skip。跨 initiator 的頻寬控制由 NI 端的 QoS Generator 緩解（Limiter 阻塞超標 request、Regulator 降低超標 initiator 的 urgency），詳見 [QoS](06_qos.md) §2 與 §6.1。

**Pluggable Allocator：** VC Allocator 與 SW Allocator 均使用抽象 `Allocator` interface，可透過 NocConfig 切換實作（`fifo` / `fixed_priority` / `round_robin` / `rotate`）。Allocator 維持 1:1 interface。

> **Testability：** 分別測試 4 種仲裁策略的正確性：FIFO 驗證到達順序、Fixed Priority 驗證高 QoS 優先、Round Robin 驗證公平輪詢、Rotate 驗證旋轉行為。

### FR-05: Buffer Management

**描述：** Input Buffer 與 Buffer State 分離設計。

| 元件 | 位置 | 職責 |
|------|------|------|
| Input Buffer | Downstream（本地） | Per-port, per-VC flit data 儲存（Push/Pop/Peek） |
| Buffer State | Upstream（遠端 tracking） | Per-VC credit counter + VC 佔用狀態追蹤 |
...
**描述：** Credit return 為 registered output（與 RTL `FF` 行為一致），從 sender 視角 latency = 1 cycle。

**Credit counter update 時機（RTL 行為對應）：**

```
Cycle N:
  Phase 4: Receiver handshake 成立（valid_i & ready_o）→ latch credit_gnt_d[vc] = 1
  Phase 5: wire_all()（本 cycle credit_o 尚未 assert）

Cycle N+1:
  Phase 1: credit_gnt_q[vc] = credit_gnt_d[vc]（registered）→ credit_o[vc] = 1
  Phase 5: wire_all() 將 credit_o 傳播至 upstream credit_i
  Phase 7: Upstream 收到 credit_i → credit_counter[vc]++
```

**Credit Starvation Detection：** 當某 VC 的 `credit_counter == 0` 持續超過 `CREDIT_TIMEOUT` cycles（預設 10,000）且該 VC 有待送 flit 時，視為異常。觸發時 log error + assert 中止（CA Model 行為），不做自動 recovery — 正常壅塞（如長 wormhole packet 佔住 downstream buffer）不會觸發此 timeout。

> **Testability：** 驗證 credit 不變量 `credit_counter + buffer_occupancy + in_flight == INPUT_BUFFER_DEPTH_PER_VC`。測試 credit=0 時 sender 停止發送。設定 `CREDIT_TIMEOUT` 驗證 starvation detection 觸發。
...
| 1.1 | 2026-03-16 | Rename VC_IMPL → FLOW_CONTROL；統一 ID_WIDTH → NODE_ID_WIDTH、NUM_VC → NUM_VIRT_CHANNELS；補充 CREDIT mode ready_o 行為、output credit_i 處理、credit registered timing；Path Lock 明確為 per-{input, vc}；FR-01 補充 invalid dst body/tail drain；VC Allocator 移除 CREDIT-only 限制；新增 §4.6 Reset Behavior；補充參數約束 |
