...
```
 ...
 Each port data path:
   Ingress → Input Buffer(depth=4) → Crossbar → Output Buffer(depth=2*) → Egress
...
| Buffer State | Per-output credit tracking（sender side），與 Input Buffer 分離 |
| Route Computer | XY routing |
| VC Allocator | HEAD flit 的 VC 分配（CREDIT mode only，pluggable） |
| SW Allocator | Per-output switch arbitration（pluggable: FIFO/Fixed/RR/Rotate） |
| Crossbar | N×N non-blocking switch fabric |
| Path Lock | Per-input wormhole lock FSM |
...
| `credit_o` | [NUM_INPUT][NUM_VC] | 1 bit each | Per-VC credit return（CREDIT mode 時使用） |
...
| 4b VA | VC Allocation | VC 分配（CREDIT mode，pluggable Allocator） |
