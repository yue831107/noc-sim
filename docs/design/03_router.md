...
Router 是 NoC Mesh 的封包轉發單元，負責接收 flit、路由決策、仲裁、crossbar switching 與轉發。每個 Router 由兩個結構對稱的子 Router 組成 — **Req Router**（AW/W/AR）與 **Rsp Router**（B/R），共用相同的 routing、arbitration、crossbar 邏輯。
...
以下參數對應 RTL module 的 `parameter`，在 virtual model 中為 compile-time 常數或 NocConfig 配置值：
...
### 3.5 Simulation Parameters（virtual model 專用）

以下參數僅存在於 virtual model，用於 pipeline delay 模擬，無直接 RTL 對應：
