...
處理 AXI 到 NoC 的 Protocol Conversion，包含地址轉換、Flit Packing/Unpacking、Reorder Buffer、ECC 與 Multicast Response Aggregation。所有參數與 [Flit Format](02_flit.md) 一致，採全固定參數設計。

---

## 1. Design Parameters

所有參數鎖定為固定值，與 [Flit Format](02_flit.md) Section 1.2 完全一致：
...
- **關鍵元件**:
  ...
  - `QoS Generator`: 產生/調整 header `qos` 值（詳見 [QoS Design](07_qos.md)）
...
採用 `{y[3:0], x[3:0]}` 座標編碼（與 [Flit Format](02_flit.md) Section 2.2.3 一致）：
...
Header bit layout 與 [Flit Format](02_flit.md) Section 2.1 完全一致：
...
| Field | Source |
|-------|--------|
| qos | QoS Generator 輸出（Bypass/Fixed/Limiter/Regulator，詳見 [QoS Design](07_qos.md)） |
...
**multicast_status encoding（與 [Flit Format](02_flit.md) Section 3.3 一致）：**
...
每個 NI 的 NMU 側擁有獨立的 32-entry RoB，管理所有 outstanding transactions 的 response reordering。RoB 架構與 [Flit Format](02_flit.md) Section 5 一致。
...
兩者獨立，用途不同（與 [Flit Format](02_flit.md) Section 5.1 一致）：
...
採用 SECDED（Single Error Correct, Double Error Detect），與 [Flit Format](02_flit.md) Section 3.6 完全一致。
...
Multicast 僅適用 Request channel（AW/W）。多個 destination 的 B response 在 **Response Router 中逐 hop 合併**（in-network reduction），Source NMU 僅收到 **1 個** 已合併的 B response。Reduction 的 Router 層實作詳見 [Router Specification](03_router.md) Section 10.3。
...
詳細的 ReductionSync、ReductionArbiter 實作見 [Router Specification](03_router.md) Section 10.3。
...
NI 內的 QoS Generator 支援四種模式，詳細設計見 [QoS Design](07_qos.md)。
...
void _QosGenerate(FlitHeader& hdr) {
    // QoS value from QoSGenerator (see 07_qos.md)
...
NI CSR 包含 QoS Generator、Performance Probe 與 Error Status，完整定義見 [QoS Design](07_qos.md) Section 4。
...
- [Router Specification](03_router.md) — CppRouter 內部 Pipeline Stages
- [Flit Format](02_flit.md) — Header/Payload bit-level 定義、ECC 設計、RoB 架構
...
- [QoS Design](07_qos.md) — QoS Generator 模式、Performance Probe、CSR Memory Map
