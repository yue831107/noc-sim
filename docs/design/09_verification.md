...
```
                    ┌─────────────────────────────┐
                    │  RTL Co-simulation (DPI-C)  │  ← C++ vs RTL 一致性
                    ├─────────────────────────────┤
...
**Match 基準：per-transaction。** 每筆 AXI transaction 的結果（data + response code）必須 C++ 與 RTL 一致，不要求每 cycle 信號完全對齊。
...
| # | 測試 | 說明 |
|---|------|------|
| 1 | Smoke test | Single write/read, C++ vs RTL transaction result match |
