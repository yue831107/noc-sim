...
本文件定義 NoC virtual model 的驗證策略、Scoreboard 機制、效能指標收集與 RTL co-simulation 驗證方法。
...
```
                    ┌─────────────────────────────┐
                    │  RTL Co-simulation (DPI-C)  │  ← virtual model vs RTL 一致性
                    ├─────────────────────────────┤
...
| IT-16 | Width Converter | up/down conversion | Data integrity through converter |
...
| Link Throughput (上限) | `FLIT_WIDTH / cycle = 408 bits/cycle = 51 bytes/cycle` |
...
```
┌─────────────────┐       ┌─────────────────┐
│  SystemVerilog  │       │   Virtual Model  │
│  RTL Testbench  │◄─────►│  (via DPI-C)    │
│  RTL Router ×N  │       │  NoC System      │
│  RTL NI ×N     │       │                 │
└────────┬────────┘       └────────┬────────┘
         ▼                         ▼
    RTL Response              Virtual Model Response
         └────────► Comparator ◄───┘
                        │
                   PASS / FAIL
```
...
**Match 基準：per-transaction。** 每筆 AXI transaction 的結果（data + response code）必須 virtual model 與 RTL 一致，不要求每 cycle 信號完全對齊。
...
| # | 測試 | 說明 |
|---|------|------|
| 1 | Smoke test | Single write/read, virtual model vs RTL transaction result match |
