...
Header 固定 56 bits，提供 Version A（No-VC）與 Version B（With-VC）。兩版本僅 bit [55:50] 的 vc_id/rsvd 配置不同，其餘欄位位置與寬度完全相同。
...
- **Version A (No-VC):** bit [55:50] 全為 rsvd（6b），無 vc_id。VC 由 signal-level 多組 valid/ready lines 實現。
- **Version B (With-VC):** bit [52:50] 為 vc_id（3b），bit [55:53] 為 rsvd（3b）。VC ID 編碼於 header，搭配 credit-based flow control。
...
AW/AR payload 不重複儲存 qos，header 為唯一來源。Response flit 繼承對應 request 的 `qos`。Router arbiter 以 `qos` 為第一優先級比較依據。詳見 [QoS Design](10_qos.md)。
...
採用 `{y[3:0], x[3:0]}` 座標編碼（x = LSB, y = MSB）：
...
**commtype 轉換規則**：Dest NI 收到 `commtype=1`（Multicast）的 AW flit 後，產生的 B response flit 設為 `commtype=2`（ParallelReduction）。Response Router 據此在 backward path 逐 hop 合併 B response。詳見 [Router Specification](02_router.md) Section 10.3 及 [NI Specification](03_network_interface.md) Section 9。

#### 2.2.8 Multicast Bounding Box (multicast_mask, 16 bits)

當 `commtype=1`（Multicast）或 `commtype=2`（ParallelReduction）時，`multicast_mask`（16 bits）採用 **RCR (Rectangle Coordinate-Range)** 編碼，以 `(min, max)` 座標對表達任意軸對齊矩形，不受 power-of-2 對齊限制。詳見 [RCR-Multicast Spec](rectangle_multicast_spec_zh-TW.md)。
...
|-------|---------------------|----------------------|
| `dst_id` [22:15] | 目標 node ID `{y, x}` | Don't care（不參與路由） |
...
Multicast 僅適用 Request channel（AW/W）。Response 使用 **ParallelReduction**（`commtype=2`）在 Response Router 中逐 hop 合併 B response，Source NMU 僅收到 **1 個**已合併的 B response。詳見 [Router Specification](02_router.md) Section 10.3 及 [NI Specification](03_network_interface.md) Section 9。

#### 2.2.9 VC ID & Reserved

**vc_id (3 bits, Version B only):**

支援最多 8 個 Virtual Channels（bit [52:50]）。用途含 Request/Response 分離避免 deadlock、traffic class 隔離（類似 AMD Versal TC-to-VC mapping）。Version A 中此 3 bits 歸入 reserved。

**Reserved bits:**

| Version | Bit Range | Width |
|---------|-----------|-------|
| A (No-VC) | [55:50] | 6 |
| B (With-VC) | [55:53] | 3 |
...
Multicast response aggregation：多個 destination 的 B response 經 ParallelReduction 在 source NI 彙整 — 全部 OKAY → MC_ALL_OK，部分非 OKAY → MC_PARTIAL，全部非 OKAY → MC_ALL_FAIL。Multicast 下 `ecc_fail` 表示任一 destination 偵測到 uncorrectable error。Aggregation 機制詳見 [NI Specification](03_network_interface.md)。
...
ASIC 實作推薦 Version B（credit-based）— wire count 不隨 VC 數增長，適合高 VC 數與 wire-constrained 場景。Version A 適用 VC ≤ 2 且無需 credit tracking 的簡化設計。
...
| Feature | This Design | AMD Versal NoC | AMBA CHI |
|---------|-------------|----------------|----------|
| Flit width | 408b (fixed) | 128b (fixed) | N/A (layer-based) |
| Header width | 56b | \~20b (est.) | N/A |
| Data width | 256b | 128b | 128/256/512b |
| Node ID | 8b (256 nodes) | Programmable | \~11b |
| Address width | 64b | 44b | 44\~52b |
| Virtual Channels | Up to 8 | 8 | Implicit (channel sep.) |
| Flow control | VR / Credit | Credit | Credit (L-Credit) |
| Multicast | Bounding box mask | N/A | Snoop broadcast |
| Data integrity | SECDED ECC | ECC | DataCheck + Poison |
| Error correction | 1-bit per granule | Yes | No |
| Topology | 2D Mesh | Programmable mesh | Point-to-point |
| Traffic class | QoS + VC | Dedicated TC (BE/LL/ISO) | Implicit |
| Parametric | No (all fixed) | Minimal | Yes |

---

## 7. Functionality Supported by Packet Design

- **XY Routing** — `src_id` (8b), `dst_id` (8b)
  - `{y,x}` 座標編碼，Router 從 `dst_id` 提取座標進行 X-first then Y routing
...
- **Virtual Channel** — `vc_id` (3b, Ver.B), `rsvd` (Ver.A)
  - Ver.A：signal-level VC（多組 valid/ready lines）
  - Ver.B：header-level VC（max 8 VCs），搭配 credit-based flow control
...
- [QoS Design](10_qos.md)
- [Router Specification](02_router.md)
- [Network Interface Specification](03_network_interface.md)
- [Physical Channel Architecture](A1_physical_channel_modes.md)
- [Fixed-Width Channel Mode](A2_fixed_width_channel_mode.md)
- [Width Converter](A3_width_converter.md)

### External References

- AMD Versal ACAP NoC — *Programmable Network on Chip LogiCORE IP Product Guide* (PG313)
- AMBA CHI — ARM, *AMBA CHI Architecture Specification* (IHI 0050)
