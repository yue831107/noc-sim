...
Req/Rsp link 寬度完全對稱（均為 410 bits），兩個 Router（Req Router / Rsp Router）可複用同一硬體模組：
...
Req Router 和 Rsp Router 共用同一 164-bit 寬度設計，可複用 RTL 模組。Wide Router 使用現有 408-bit 寬度設計。所有 Router 共用相同 routing algorithm 與 arbitration policy。
