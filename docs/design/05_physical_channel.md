...
兩個 Router（Req Router / Rsp Router）共用相同的 routing algorithm（XY routing）、arbitration policy（pluggable arbitration scheme，詳見 06_qos.md）、crossbar structure，僅處理的 flit 類型不同（依 header `axi_ch` 欄位區分）。
...
| AW/AR 互相阻塞 | 競爭同一 output port | Switch Allocator 仲裁，落選者延遲 1 cycle |
...
| NI QoS Generator | Limiter/Regulator 控制各 initiator 頻寬 | [QoS](06_qos.md) §2 |
...
Req Router 和 Rsp Router 共用同一 164-bit 寬度設計，可複用 RTL 模組。Wide Router 使用現有 400-bit 寬度設計。所有 Router 共用相同 routing algorithm 與 pluggable arbitration scheme（FIFO/Fixed/RR/Rotate，詳見 06_qos.md）。
...
- [QoS Design](06_qos.md) — Pluggable arbitration policy、Limiter/Regulator
