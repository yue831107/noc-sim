...
> **Implementation Note：** Regulator 的 `response_bytes` 來自 NMU 的 response path。C++ model 中，QoS Generator 需要 NMU 提供 `on_response(bytes)` callback 或每 cycle 輪詢 response counter。
