# Fixed-Width Unified Flit Design

本文件說明 NoC 採用統一 408-bit 固定寬度 flit 的設計動機、結構細節、效率分析，以及與 per-channel variable-width 方案的比較。所有參數與欄位定義以 [Flit Format](04_flit.md) 為準。
...
## 2. Header Structure (56 bits)

> **⚠ 注意：** 本 section 的 header bit allocation 為早期設計版本，與 [04_flit.md](04_flit.md) 的最終定義不一致。所有 header 欄位定義以 04_flit.md Section 2 為準。本文的固定寬度效率分析結論仍然有效。

Header 固定 56 bits，提供兩個版本（No-VC / With-VC），詳見 [04_flit.md Section 2](04_flit.md)。以下為 bit allocation 摘要：

| Bit Range | Field | Width | Description |
|-----------|-------|-------|-------------|
| [2:0] | dst_x | 3 | Destination X |
| [5:3] | dst_y | 3 | Destination Y |
| [8:6] | src_x | 3 | Source X |
| [11:9] | src_y | 3 | Source Y |
| [19:12] | txn_id | 8 | Transaction ID |
| [21:20] | flit_type | 2 | HEAD/BODY/TAIL/HEAD_TAIL |
| [24:22] | axi_ch | 3 | AXI channel type (AW/W/AR/B/R) |
| [28:25] | qos | 4 | QoS level (0\~15) |
| [29] | last | 1 | Last flit in packet |
| [31:30] | port_id | 2 | LOCAL port ID (0\~3) |
| [33:32] | commtype | 2 | Communication type |
| [49:34] | multicast_mask | 16 | Multicast target mask |
| [52:50] | vc_id ‡ | 3 | Virtual Channel ID (Ver.B only) |
| [55:53] | rsvd ‡ | 3 | Reserved |
| | **Total** | **56** | |

‡ Version A（No-VC）：bit [55:50] 全為 rsvd（6b）。Version B（With-VC）：bit [52:50] 為 vc_id（3b），bit [55:53] 為 rsvd（3b）。

### 2.1 Bit Layout (Version A)

> **⚠ 注意：** 以下 bit layout 為早期設計版本，欄位名稱與排列已過時。最終 header bit layout 請參見 [04_flit.md Section 2.1](04_flit.md)。

```
 55  50 49               34 33 32 31 30  29  28  25 24 22 21 20 19    12 11  9 8   6 5   3 2   0
┌──────┬─────────────────┬─────┬─────┬────┬──────┬─────┬──────┬────────┬─────┬─────┬─────┬─────┐
│ rsvd │ multicast_mask  │comm │port │last│ qos  │ axi │flit  │ txn_id │src_y│src_x│dst_y│dst_x│
│  6b  │      16b        │type │id 2b│ 1b │  4b  │ch 3b│type  │   8b   │  3b │  3b │  3b │  3b │
│      │                 │ 2b  │     │    │      │     │  2b  │        │     │     │     │     │
└──────┴─────────────────┴─────┴─────┴────┴──────┴─────┴──────┴────────┴─────┴─────┴─────┴─────┘
```
...
- 高頻存取欄位（qos、axi\_ch）置於 LSB 端，第一級 pipeline 僅需部分 decode 即可完成 arbitration 與 channel 分辨
- Multicast 與 reserved 置於 MSB 端，未來擴展不影響既有 bit position
- Router 依據 `axi_ch` 判定 flit 所屬 Request 或 Response network
- Node ID 採 `{y[3:0], x[3:0]}` 座標編碼，支援最大 16×16 mesh（256 nodes）
...
兩個子 Router 結構完全相同，僅處理的 flit 類型不同。此對稱性得益於統一 flit width — ReqRouter 與 RspRouter 可共用相同的 RTL module（參數化 instance），大幅減少設計與驗證工作量。
...
每方向一條 410-bit link，Req 與 Rsp 各一條，雙方向共 4 條。兩版 header（Version A / Version B）的 physical link 寬度相同，版本差異僅在 header 內部 bit 解釋。

---

## Related Documents

- [Flit Format](04_flit.md) — 完整 flit 參數與欄位定義
- [Physical Channel Architecture](A1_physical_channel_modes.md) — 雙通道架構詳述
- [Width Converter](A3_width_converter.md) — 不同 AXI bus 寬度轉換
- [Router Specification](02_router.md) — Router 內部架構
- [Network Interface Specification](03_network_interface.md) — NI 設計
