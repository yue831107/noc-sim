...
# NoC Virtual Model - Project Goals

## Executive Summary

本專案建立 NoC (Network-on-Chip) 的 **Virtual Cycle-Accurate Behavior Model**，同時作為 **Pre-silicon 效能評估平台** 與 **RTL Co-simulation Golden Reference**。

同一組 Input Pattern 餵入 virtual model 和 RTL，virtual model 的 Output 作為 Golden，與 RTL Output 進行 byte-exact / cycle-accurate 比對。
...
本專案目標是建立一套 NoC 的 Virtual Behavior Model，架構規格如下：
...
在 RTL 實作之前，以 virtual cycle-accurate model 進行 traffic pattern 掃描與參數空間探索。

| 需求 | 說明 |
|------|------|
| **Cycle-Accurate 模擬** | virtual model，模擬速度高於 RTL simulation |
...
> **Key Takeaway**: 在 RTL 之前，用 virtual model 完成設計空間探索。

---

### 目標 B: RTL 共模驗證 (RTL Co-Simulation / Golden Reference)

virtual model 作為 RTL 的 golden reference，確保硬體實作與行為模型一致。

| 需求 | 說明 |
|------|------|
| **Cycle-Accurate** | 與 RTL 逐 cycle 比對，結果完全一致 |
| **DPI-C 介面** | DPI-C Bridge 每 cycle 與 RTL module 交換 port 信號（flit + valid + ready + credit） |
| **Hot-Swap** | 單一 Router 或 NI 可替換為 RTL，其餘維持 virtual |
| **Response 驗證** | 攔截 RTL response，與 virtual model 預期結果比對 |
| **Data Integrity** | Golden Data 機制，支援 MULTICAST |
| **Waveform Dump** | 可選擇輸出 virtual model 內部狀態供 waveform 分析 |

> **Key Takeaway**: virtual model = Golden Reference，支援 per-instance 替換 RTL 進行漸進驗證。
...
| **Public API** | NocSystem Public API | 5 組 API（`noc_api.h`），詳見 §4.2 |
...
> **Key Takeaway**: User Code 透過 Public API 操作 Internal Components；Co-Sim Bridge 負責 virtual model ↔ RTL 信號交換。

### 3.2 Abstract Interface + Hot-Swap

透過 `Router_Interface<Mode>` 和 `NI_Interface<Mode>` 抽象介面，支援：
- **VirtualRouter / VirtualNI**: 純 virtual 行為模型（用於獨立效能模擬）
- **RouterDpiBridge / NIDpiBridge**: 透過 DPI-C 橋接到 RTL（用於 co-simulation）
- 可 per-instance 替換：例如 15 個 VirtualRouter + 1 個 RouterDpiBridge
...
> **Key Takeaway**: 抽象介面 = 可插拔邊界，virtual 與 RTL 實作共用同一合約。
...
### 3.6 Virtual Model 與 RTL 分工

| 負責方 | 職責 |
|--------|------|
| **Virtual Model** | Routing 決策、Flow Control、Flit 組裝/解析、Arbitration、Buffer 管理 |
| **RTL** | 物理通道傳輸、時序對齊、信號驅動 |
| **DPI-C** | 薄層橋接 — DPI-C Bridge 每 cycle 呼叫 `noc_set_port_input()` / `noc_get_port_output()` 交換 port 信號 |

> **Key Takeaway**: virtual model 做邏輯，RTL 做信號，DPI-C 只搬資料。
...
### 4.2 NocSystem Public API（5 組）

| Group | 名稱 | 主要 API |
|-------|------|---------|
| **A** | Construction | `NocSystem(config)`, `load_memory()`, `load_traffic()` |
| **B** | Transaction | `submit_write()`, `submit_read()`, `submit_multicast_write()` → `TxnHandle`, `is_complete()`, `get_status()`, `get_read_data()`, `set_completion_callback()` |
| **C** | Simulation | `process_cycle()`, `run(N)`, `run_until_idle()`, `run_all()` |
| **D** | Metrics | `get_metrics()`, `verify()` |
| **E** | Debug & Output | `get_router()`, `get_ni()`, `dump_state()`, `generate_golden()` |
...
> **Key Takeaway**: 5 組 API 覆蓋完整 lifecycle — 建構 → 提交與查詢 → 執行 → 驗證 → Debug。

### 4.3 Internal Components

| 元件 | 職責 |
|------|------|
| `NocSystem` | 頂層系統，協調所有元件，暴露 5 組 API |
...
| 元件 | Abstract Interface | Virtual Model | DPI-C Bridge |
|------|-------------------|-----------|-------------|
...
**固定 virtual 實作**: TrafficManager, Mesh, Channel, Scoreboard, MetricsCollector, HostMemory/LocalMemory
...
```cpp
extern "C" {
    // 每 cycle 將 virtual model 側信號送入 RTL module 的 port
    void noc_set_port_input(int node, int port, int ch,
                            int valid, const uint8_t* flit, int credit);

    // 每 cycle 將 RTL module 的 port 輸出讀回 virtual model
...
| Phase | 名稱 | 目標 | 關鍵交付物 |
|-------|------|------|-----------|
| **1** | Core Library | 獨立 virtual model，可執行效能模擬 | Flit, Channel, Router, NI, Mesh, NocSystem, TrafficManager |
...
- [ ] 8-Phase tick 同步機制（virtual model ↔ RTL）
