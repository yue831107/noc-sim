...
# NoC Cycle-Approximate Model - Project Goals

## Executive Summary

本專案建立 NoC (Network-on-Chip) 的 **Cycle-Accurate CA Model**，同時作為 **Pre-silicon 效能評估平台** 與 **RTL Co-simulation Golden Reference**。

同一組 Input Pattern 餵入 CA Model 和 RTL，CA Model 的 Output 作為 Golden，與 RTL Output 進行 byte-exact / cycle-accurate 比對。
...
本專案目標是建立一套 NoC 的 CA Model，架構規格如下：
...
在 RTL 實作之前，以 Cycle-Approximate (CA) Model 進行 traffic pattern 掃描與參數空間探索。

| 需求 | 說明 |
|------|------|
| **Cycle-Accurate 模擬** | CA Model，模擬速度高於 RTL simulation |
...
> **Key Takeaway**: 在 RTL 之前，用 CA Model 完成設計空間探索。

---

### 目標 B: RTL 共模驗證 (RTL Co-Simulation / Golden Reference)

CA Model 作為 RTL 的 golden reference，確保硬體實作與行為模型一致。

| 需求 | 說明 |
|------|------|
| **Cycle-Accurate** | 與 RTL 逐 cycle 比對，結果完全一致 |
| **DPI-C 介面** | DPI-C Bridge 每 cycle 與 RTL module 交換 port 信號（flit + valid + ready + credit） |
| **Hot-Swap** | 單一 Router 或 NI 可替換為 RTL，其餘維持 CA Model |
| **Response 驗證** | 攔截 RTL response，與 CA Model 預期結果比對 |
| **Data Integrity** | Golden Data 機制，支援 MULTICAST |
| **Waveform Dump** | 可選擇輸出 CA Model 內部狀態供 waveform 分析 |

> **Key Takeaway**: CA Model = Golden Reference，支援 per-instance 替換 RTL 進行漸進驗證。
...
> **Key Takeaway**: User Code 透過 Public API 操作 Internal Components；Co-Sim Bridge 負責 CA Model ↔ RTL 信號交換。

### 3.2 Abstract Interface + Hot-Swap

透過 `Router_Interface<Mode>` 和 `NI_Interface<Mode>` 抽象介面，支援：
- **CppRouter / CppNI**: 純 CA 行為模型（用於獨立效能模擬）
...
> **Key Takeaway**: 抽象介面 = 可插拔邊界，CA Model 與 RTL 實作共用同一合約。
...
### 3.6 CA Model 與 RTL 分工

| 負責方 | 職責 |
|--------|------|
| **CA Model** | Routing 決策、Flow Control、Flit 組裝/解析、Arbitration、Buffer 管理 |
| **RTL** | 物理通道傳輸、時序對齊、信號驅動 |
| **DPI-C** | 薄層橋接 — DPI-C Bridge 每 cycle 呼叫 `noc_set_port_input()` / `noc_get_port_output()` 交換 port 信號 |

> **Key Takeaway**: CA Model 做邏輯，RTL 做信號，DPI-C 只搬資料。
...
| 元件 | Abstract Interface | CA Model | DPI-C Bridge |
|------|-------------------|-----------|-------------|
...
**固定 CA 實作**: TrafficManager, Mesh, Channel, Scoreboard, MetricsCollector, HostMemory/LocalMemory
...
| Phase | 名稱 | 目標 | 關鍵交付物 |
|-------|------|------|-----------|
| **1** | Core Library | 獨立 CA Model，可執行效能模擬 | Flit, Channel, Router, NI, Mesh, NocSystem, TrafficManager |
...
- [ ] 8-Phase tick 同步機制（CA Model ↔ RTL）
